...
###############################################################################
# WHITELIST SPEC (AUTHOR-FACING)
###############################################################################

# This file is TRACKED and human-authored.
# It is a WHITELIST of paths that belong to this layer template.
#
# Example:
#   app/**
#   src/shared/**
...
###############################################################################
# WHITELIST COMPILATION (WITH EXCLUDES)
###############################################################################
# (unchanged)
sync_exclude() {
  [[ -f "$geetinclude_SPEC" ]] || return 0
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<EOF
Usage:
  $SCRIPT_DIR/git.sh clone <repo> [dir] [--recurse-submodules] [-- post-init-args...]

Examples:
  $SCRIPT_DIR/git.sh clone git@github.com:me/template.git MyApp
  $SCRIPT_DIR/git.sh clone --recurse-submodules git@github.com:me/template.git
  $SCRIPT_DIR/git.sh clone git@github.com:me/template.git MyApp -- --app-name "My Cool App"

Notes:
  - After cloning, this runs: $INIT_SH
...
Whitelist spec:
  - source:   $geetinclude_SPEC
  - compiled: $EXCLUDE_FILE

Usage:
  $SCRIPT_DIR/git.sh clone <repo> [dir] [--recurse-submodules] [-- post-init-args...]
  $SCRIPT_DIR/git.sh init [remote] [branch]
  $SCRIPT_DIR/git.sh pull [remote] [branch]
  $SCRIPT_DIR/git.sh status | diff | log
  $SCRIPT_DIR/git.sh add -A
  $SCRIPT_DIR/git.sh commit -m "msg"
  $SCRIPT_DIR/git.sh push [remote] [branch]
  $SCRIPT_DIR/git.sh <any git args>

Notes:
- 'clone' runs git clone + init.sh; args after '--' are passed to post-init hook
